 ...
 *        the vehicle stays in a range of [+-5 cm] for x and y and [-11,+5 cm] for z
 *          
...
void Director::local_position_handler(px4_msgs::msg::VehicleOdometry msg)
{
    if ((state == Status::MOVE) || (state == Status::HOLD))
    {
        ...
        bool x_pos = sqrt(pow(msg.position[0]-setpoint[0],2))<0.10; // sphere with 10 cm radius
        bool y_pos = sqrt(pow(msg.position[1]-setpoint[1],2))<0.10;
